...
export const fetchGoogleLogin = async () => {
    try {
        ...
        const response = await axios.post('https://api.suzueyume.id.vn/login/google', { googleIdToken: idToken });
...
export const fetchUpdateProfile = async (profileData, avatar) => {
    try {
      ...
      if (avatar && avatar.uri && avatar.type && avatar.type.startsWith('image/')) {
