...
const fetchDistances = async (source, features, controller, chunkSize = 20) => {
...
const fetchDurations = async (source, features, controller, chunkSize = 20) => {
    const BASE_URL = "https://api.suzueyume.id.vn/osrm/table/v1/driving";
    ...
    for (const chunk of destinationChunks) {
        ...
        while (true) {
            ...
            try {
                ...
                allDurations.push(...response.data.durations[0].slice(1));
                break;
            } catch (error) {
                if (axios.isCancel(error)) {
                    return null
...
export { fetchDistances, fetchDistance, fetchDurations };